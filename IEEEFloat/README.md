...
IEEE-754 represents a floating-point number with a sign field, an exponent
field, and a fraction field. The sign field is separate from the magnitude
fields. For zero, both the exponent and fraction fields are all zero. The sign
bit can still be either `0` or `1`, producing `+0.0f` and `-0.0f`.
