 ...
 *
 * This class provides easy-to-use functions for comparing different GEMM
 * implementations across CPU (naive, optimized, ulmBLAS) and GPU versions.
 */
...
public:
   ...
   */
  template <typename T>
  static double runGEMM(Implementation impl, const std::vector<T> &A,
                        const std::vector<T> &B, std::vector<T> &C, int M,
                        int N, int K) {
    C.resize(M * N);
    ...
    case Implementation::CPU_TILED_8:
      ...
      throw std::runtime_error(
          "CPU_TILED_8 is not supported when AVX512 is enabled");
#else
      gemm::TiledMatMatMulInternalTiledPadded<T, 8>(A.data(), B.data(),
                                                    C.data(), M, N, K);
#endif
      break;

    case Implementation::CPU_TILED_16:
      gemm::TiledMatMatMulInternalTiledPadded<T, 16>(A.data(), B.data(),
                                                     C.data(), M, N, K);
      break;
    ...
    case Implementation::CPU_DGEMM_NN:
      if constexpr (std::is_same_v<T, double>) {
        gemm::gemm_nn<double>(M, N, K, 1.0, A.data(), K, B.data(), N, 0.0,
                              C.data(), N);
      } else {
    ...
    case Implementation::GPU_CUDA:
      if constexpr (std::is_same_v<T, float>) {
        ...
        throw std::runtime_error(
            "GPU_CUDA currently only supports float precision");
      }
      break;
    }
  ...
  template <typename T>
  static void
  compareImplementations(const std::vector<Implementation> &implementations,
                         const std::vector<T> &A, const std::vector<T> &B,
                         int M, int N, int K, int num_runs = 5) {
    ...
    for (auto impl : implementations) {
      ...
      std::cout << std::setw(20) << name << ": " << std::setw(8) << avg_time
                << " ms, " << std::setw(6) << gflops << " GFLOPS\n";
    }
    ...
    if (times.size() > 1) {
      ...
      for (size_t i = 1; i < times.size(); ++i) {
        ...
        std::cout << names[i] << " vs " << names[0] << ": " << speedup
                  << "x speedup\n";
      }
    }
    std::cout << "\n";
  ...
  static void quickCPUvsGPUComparison(int M = 256, int N = 256, int K = 256) {
    static_assert(std::is_same_v<T, float>,
                  "GPU implementation currently only supports float");
    ...
    std::vector<Implementation> impls = {
        Implementation::CPU_NAIVE, Implementation::CPU_TILED_16,
        Implementation::CPU_ULMBLAS, Implementation::GPU_CUDA};
...
private:
  static std::string getImplementationName(Implementation impl) {
    ...
    case Implementation::CPU_NAIVE:
      return "CPU Naive";
    case Implementation::CPU_TILED_8:
      return "CPU Tiled 8x8";
    case Implementation::CPU_TILED_16:
      return "CPU Tiled 16x16";
    case Implementation::CPU_ULMBLAS:
      return "CPU ulmBLAS";
    case Implementation::CPU_DGEMM_NN:
      return "CPU dgemm_nn";
  ...
  static bool verifyResults(const std::vector<T> &reference,
                            const std::vector<T> &test, T tolerance) {
