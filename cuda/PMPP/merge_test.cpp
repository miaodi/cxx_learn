...
protected:
  ...
  void verifyMergeShared(const std::vector<int> &a,
                         const std::vector<int> &b) {
    std::vector<int> expected(a.size() + b.size());
    std::vector<int> actual(a.size() + b.size());

    std::merge(a.begin(), a.end(), b.begin(), b.end(), expected.begin());
    PMPP::merge_shared(const_cast<int *>(a.data()),
                       static_cast<int>(a.size()), const_cast<int *>(b.data()),
                       static_cast<int>(b.size()), actual.data());
